...
void RelicZone::triggerOnDamageDealt(int& scoredPts, RoundTracker& state) {
	for (auto& relic : relics) {
		if (relic->isDisabled())relic->onDamageDealt(scoredPts, state);
	}
}
void RelicZone::triggerOnRoundStart(RoundTracker& state) {
	for (auto& relic : relics) {if (relic->isDisabled()) continue; relic->onRoundStart(state);}
}
void RelicZone::triggerOnRoundEnd(RoundTracker& state) {
	for (auto& relic : relics){if (relic->isDisabled()) relic->onRoundEnd(state);}
}