...
private:
  ...
  std::function<void(int, RoundTracker&)> pendingCardSelectionCallback = nullptr;
...
public:
  ...
  void requestXPrompt(std::function<void(int, RoundTracker&)> cb) { pendingXCallback = std::move(cb); }
  ...
  void requestYesNoPrompt(std::string msg, std::function<void(bool, RoundTracker&)> cb) {
  ...
  bool isCardSelectionPromptActive() const { return pendingCardSelectionCallback != nullptr; }
  std::string getSelectionMessage() const { return selectionPromptMessage; }
  const std::vector<std::shared_ptr<Card>>& getSelectionChoices() const { return selectionChoices; }
  
  void requestCardSelection(std::string msg, std::vector<std::shared_ptr<Card>> choices, std::function<void(int, RoundTracker&)> cb) {
  ...
  void resolveCardSelection(int index) {
      if (pendingCardSelectionCallback) {
          ...
          // It's up to the caller to validate the index if needed, or we just pass it blindly
          cb(index, *this);
      }
  }
